CREATE OR REPLACE FUNCTION public.datosestudiantes(fechainicial date, fechafinal date)
...
SELECT ag.codigo_proyecto                   AS ID_SUB_PROYECTO_CURSO,
                    ...
                    (select CASE
                        ...
                        END
                    FROM administracion_descuentoaplicado descap
                        JOIN administracion_descuento des
                        ON descap.descuento_id = des.id
                        WHERE descap.estado_descuento = 2
                        ...
                        AND preinscr.id = descap.preinscripcion_generada_id
                        ) AS Descuento,
    ...
    FROM administracion_profile AS perf
        ...
        left join administracion_descuentoaplicado descap
            on preinscr.id = descap.preinscripcion_generada_id
        ...
        join administracion_matricula am ON (perf.id = am.estudiante_id and am.preinscripcion_generada_id = preinscr.id)
        join administracion_grupoacademico ag ON (am.grupo_id = ag.id AND ag."horarioCurso_id" = prehc.horario_cupo_id)
        join administracion_horariocurso ah ON ag."horarioCurso_id" = ah.id
        join administracion_curso cur ON ah.curso_id = cur.id
      ...
      WHERE preinscr.fecha_preinscripcion between fechaInicial and fechaFinal
      AND preinscr.requiere_facturacion IS TRUE
      AND preinscr.estado_preinscripcion = 1
    GROUP BY ID_SUB_PROYECTO_CURSO, preinscr.id, perf.tipo_documento_id, perf.numero_documento, perf.primer_nombre, perf.segundo_nombre,
             perf.primer_apellido, perf.segundo_apellido, perf.genero_sexual, perf.estado_civil, perf.fecha_nacimiento,
             apnac.id, arnac.id, acnac.id, perf.nivel_formacion, perf.es_egresado, perf.tipo_vinculacion_un,
             perf.telefono_fijo, perf.telefono_celular, au.email, perf.direccion_residencia, ap.id, ar.id, ac.id,
             ao.tarifa, descuento.porcentaje
    ORDER BY ID_SUB_PROYECTO_CURSO,
             Tipo_Documento,
             sexo_biologico, estado_civil,
             Departamento_Nacimiento, Ciudad_Naciemiento,
             Nivel_Formacion,
