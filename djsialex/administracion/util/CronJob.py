...
def Inscritos_Pendientes():
    ...
    periodo = Periodo.objects.filter(activo = True, fecha_pendientes = date.today() )
    if periodo:
        pendientes = PreinscripcionHorarioCurso.objects.filter(estado_preinscripcion = 3,
                                                               horario_cupo__curso__oferta_academica__periodo__in=periodo)
        header = ['#','Horario Curso','Tipo documento', 'Numero documento', 'Nombre estudiante','Correo electrónico','Estado','Fecha Inscripcion','Valor inscripcion']
        ...
        ['rodrigo.prada@cobiscorp.com','roprale@gmail.com']
        )
